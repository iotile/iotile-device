...
import {IndividualReport, SignatureStatus, SignedListReport, SignedReportSelectors } from "../../src";


export function createIndividualReport (uuid, stream, value, sentTime, rawTime) {
...
export function expectIndividual(inReport, uuid, stream, value, sentTime, rawTime) {
...
export function expectSequential(inReport, uuid, stream: string, count, streamer) {
...
export function feedInPieces(report: ArrayBuffer, handler: (ArrayBuffer) => void, size: number) {
    for (let i = 0; i < report.byteLength; i += size) {
        handler(report.slice(i, i+size));
    }
}

export function createReading (stream, timestamp, value, id) {
    return {
...
export function createHashListReport (uuid, reportID, streamer, sentTime, readings, selector?: number) {
